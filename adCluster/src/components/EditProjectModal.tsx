...
const EditProjectModal: React.FC<EditProjectModalProps> = ({ project, onClose, onUpdate }) => {
  ...
  // Set today's date as default for startDate if it's not set
  useEffect(() => {
    if (!startDate) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update the project
      const updatedProject = await updateProject(project.prjid, updateData);
  ...
  const handleCancel = () => {
    if (!loading && window.confirm('프로젝트 수정을 취소하시겠습니까?')) {
      onClose();
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="modal-content bg-white rounded-lg w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        <div className="modal-header p-5 border-b border-gray-200 flex justify-between items-center">
          <div className="modal-title text-xl font-semibold text-gray-800">연구 프로젝트 수정</div>
          <button 
            className="close-btn text-gray-500 hover:text-gray-700 text-2xl"
          ...
          >
            {loading ? (
              <>
                ...
                <span className="hidden md:inline">수정 중...</span>
              </>
            ) : (
              <>
                <i className="fas fa-save mr-2"></i>
                <span className="hidden md:inline">프로젝트 수정</span>
              </>
            )}
          </button>
        </div>
