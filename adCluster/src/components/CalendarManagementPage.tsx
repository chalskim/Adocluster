...
const CalendarManagementPage: React.FC = () => {
  ...
  const [showWeeklyView, setShowWeeklyView] = useState(false); // For weekly view mockup
  ...
  // 이벤트 저장 핸들러
  const handleSaveEvent = useCallback((eventData: Omit<CalendarEvent, 'us_id' | 'us_createdat' | 'us_updatedat' | 'us_deletedat' | 'us_isdeleted'>) => {
    addEvent(eventData);
  }, [addEvent]);

  // 이벤트 업데이트 핸들러
  const handleUpdateEvent = useCallback((eventId: number, updates: Partial<CalendarEvent>) => {
  ...
  const getFilteredEvents = useCallback(() => {
    ...
    if (searchTerm) {
      filteredEvents = filteredEvents.filter(event =>
        event.us_title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (event.us_description && event.us_description.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (event.us_location && event.us_location.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }

    // 카테고리 필터
    if (filterCategory !== 'all') {
      filteredEvents = filteredEvents.filter(event => event.us_category === filterCategory);
    }
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b border-gray-200 p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gray-800">일정 관리</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
          <div className="bg-blue-50 p-3 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs md:text-sm text-gray-600">전체 일정</p>
          ...
          <div className="bg-green-50 p-3 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs md:text-sm text-gray-600">오늘 일정</p>
          ...
          <div className="bg-orange-50 p-3 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs md:text-sm text-gray-600">예정된 일정</p>
                  ...
                  {selectedDate ? (
                    (() => {
                      const dayEvents = getEventsForDate(selectedDate!).filter(event => {
                        ...
                        if (filterCategory !== 'all') {
                          matches = matches && (event.us_category === filterCategory);
                        }
                        if (filterPriority !== 'all') {
                      ...
                      return dayEvents.length > 0 ? (
                            ...
                            >
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  {event.isAllDay === false && event.us_starttime && (
                                    <p className="text-xs text-gray-600 mt-1">
                                      {event.us_starttime}
                                      {event.us_endtime && ` - ${event.us_endtime}`}
                                    </p>
                                  )}
                                  {event.us_location && (
                                  ...
                                  <div className="flex items-center gap-1 mt-2">
                                    ...
                                    <span className="text-xs px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded-full">
                                      {event.us_category === 'WORK' ? '업무' :
                                       event.us_category === 'PERSONAL' ? '개인' :
                                       event.us_category === 'MEETING' ? '회의' :
                                       event.us_category === 'DEADLINE' ? '마감일' : '기타'}
                                    </span>
                                  </div>
                                </div>
                              </div>
                        ...
                        <div className="text-center text-gray-500 py-6">
                          ...
                          <p className="text-sm">이 날짜에는 일정이 없습니다.</p>
                        </div>
                      );
                    })()
                  ) : (
                    <div className="text-center text-gray-500 py-6">
                      ...
                      <p className="text-sm">해당 날짜의 일정을 확인하세요.</p>
                    </div>
                  )}
                </div>
                {selectedDate && (
