...
var PaginationPlus = Extension.create({
  ...
  onCreate() {
    ...
    const refreshPage = (targetNode2) => {
      var _a;
      const paginationElement = targetNode2.querySelector("[data-rm-pagination]");
      if (paginationElement) {
        ...
        if (lastPageBreak) {
          ...
          targetNode2.style.minHeight = `${minHeight}px`;
        }
      }
    };
...
var calculatePageCount = (view, pageOptions) => {
  var _a;
  const editorDom = view.dom;
  ...
  if (paginationElement) {
    ...
    if (lastElementOfEditor && lastPageBreak) {
      const lastPageGap = lastElementOfEditor.getBoundingClientRect().bottom - lastPageBreak.getBoundingClientRect().bottom;
      if (lastPageGap > 0) {
...
function createDecoration(state, pageOptions, isInitial = false) {
  const pageWidget = Decoration.widget(0, (view) => {
    ...
    const pageBreakDefinition = ({ firstPage: firstPage2 = false }) => {
      ...
      page2.style.clear = "both";
      page2.style.marginTop = firstPage2 ? `calc(${_pageHeaderHeight}px + ${_pageHeight}px)` : _pageHeight + "px";
      ...
      pageContainer.append(page2, pageBreak);
